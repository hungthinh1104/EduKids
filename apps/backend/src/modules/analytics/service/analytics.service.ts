...
export class AnalyticsService {
   ...
   */
  async getDailyActiveUsers(queryDto: AnalyticsQueryDto): Promise<DAUResponseDto | InsufficientDataResponseDto> {
    const { startDate, endDate, timeRange: timeRangeLabel } = this.getDateRange(queryDto.timeRange, queryDto.startDate, queryDto.endDate);
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      const activeUsersList = await this.redisAnalytics.getDailyActiveUsersList(date);
      ...
      const newUsers = await this.analyticsRepository.getNewUsersCount(date, nextDay);
    ...
    return {
      ...
      dailyData,
      averageDAU: dailyData.length > 0 ? Math.round(sumDAU / dailyData.length) : 0,
      peakDAU: maxDAU,
      totalUniqueUsers,
    };
  }
   ...
   */
  async getSessionLength(queryDto: AnalyticsQueryDto): Promise<SessionLengthResponseDto | InsufficientDataResponseDto> {
    const { startDate, endDate, timeRange: timeRangeLabel } = this.getDateRange(queryDto.timeRange, queryDto.startDate, queryDto.endDate);
    ...
    const overallAverageSeconds = totalSessions > 0 ? Math.round(totalDuration / totalSessions) : 0;
   ...
   */
  async getContentPopularity(queryDto: ContentPopularityQueryDto): Promise<ContentPopularityResponseDto | InsufficientDataResponseDto> {
    const { startDate, endDate, timeRange: timeRangeLabel } = this.getDateRange(queryDto.timeRange, queryDto.startDate, queryDto.endDate);
    ...
    const contentViews = new Map<string, { viewCount: number; uniqueUsers: Set<string> }>();

    for (const dateStr of dates) {
      const date = new Date(dateStr);
      
      // Get top content from Redis for each content type
      const types = contentType ? [contentType] : ['TOPIC', 'VOCABULARY', 'QUIZ'];
      
      for (const type of types) {
        const topContent = await this.redisAnalytics.getTopContentByViews(date, type as any, 100);
    ...
    for (let i = 0; i < sortedContent.length; i++) {
      ...
      if (type === 'TOPIC') {
        const topics = await this.analyticsRepository.getTopicsByIds([contentId]);
        contentName = topics[0]?.name || 'Unknown Topic';
      } else if (type === 'VOCABULARY') {
        const vocabs = await this.analyticsRepository.getVocabulariesByIds([contentId]);
        contentName = vocabs[0]?.word || 'Unknown Vocabulary';
      } else if (type === 'QUIZ') {
        const quizzes = await this.analyticsRepository.getQuizzesByIds([contentId]);
        contentName = quizzes[0]?.question || 'Unknown Quiz';
      }

      // Get average time spent (from last date for simplicity)
      const lastDate = new Date(dates[dates.length - 1]);
      const timeSpentData = await this.redisAnalytics.getContentTimeSpent(contentId, type as any, lastDate);
      const averageTimeSpent = timeSpentData.length > 0
        ? Math.round(timeSpentData.reduce((acc, t) => acc + t, 0) / timeSpentData.length)
        : 0;

      // Get completion rate
      const completionStats = await this.redisAnalytics.getContentCompletionStats(contentId, type as any, lastDate);
      const completionRate = completionStats.rate;

      // Get unique viewers (approximate)
      let uniqueUsers = 0;
      for (const dateStr of dates) {
        const count = await this.redisAnalytics.getContentUniqueViewers(contentId, type as any, new Date(dateStr));
        uniqueUsers += count;
      }
  ...
  async getDashboardSummary(): Promise<DashboardSummaryDto> {
    ...
    const yesterdayDAU = await this.redisAnalytics.getDailyActiveUsers(yesterday);
    const dauChangePercent = yesterdayDAU > 0 
      ? ((currentDAU - yesterdayDAU) / yesterdayDAU) * 100 
      : 0;
    ...
    // Get new users today
    const newUsersToday = await this.analyticsRepository.getNewUsersCount(today, tomorrow);

    // Get today's session data
    const sessionDurations = await this.redisAnalytics.getSessionDurations(today);
    const averageSessionLength = sessionDurations.length > 0
      ? Math.round(sessionDurations.reduce((acc, d) => acc + d, 0) / sessionDurations.length)
      : 0;

    // Get top content today
    const topContentData = await this.redisAnalytics.getTopContentByViews(today, 'TOPIC', 1);
    let topContent: ContentPopularityItemDto | null = null;

    if (topContentData.length > 0) {
      const content = topContentData[0];
      const topics = await this.analyticsRepository.getTopicsByIds([content.contentId]);
      const uniqueUsers = await this.redisAnalytics.getContentUniqueViewers(content.contentId, 'TOPIC', today);
      const timeSpentData = await this.redisAnalytics.getContentTimeSpent(content.contentId, 'TOPIC', today);
      const averageTimeSpent = timeSpentData.length > 0
        ? Math.round(timeSpentData.reduce((acc, t) => acc + t, 0) / timeSpentData.length)
        : 0;
    ...
    const topicViews = await this.redisAnalytics.getTopContentByViews(today, 'TOPIC', 100);
    const vocabViews = await this.redisAnalytics.getTopContentByViews(today, 'VOCABULARY', 100);
    const quizViews = await this.redisAnalytics.getTopContentByViews(today, 'QUIZ', 100);
   ...
   */
  private insufficientDataResponse(minimum: number, available: number): InsufficientDataResponseDto {
    return {
      message: 'Not enough data yet',
      minimumRequired: minimum,
      currentAvailable: available,
      suggestion: 'Please try a shorter time range or wait for more data to accumulate',
    };
  }
}
