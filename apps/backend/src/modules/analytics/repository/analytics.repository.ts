...
/**
 * Repository for analytics data persistence and aggregation
...
export class AnalyticsRepository {
   ...
   */
  async getNewUsersByDate(startDate: Date, endDate: Date): Promise<Array<{ date: string; count: number }>> {
    const users = await this.prisma.$queryRaw<Array<{ date: string; count: bigint }>>`
      SELECT 
  ...
  async getQuizzesByIds(quizIds: string[]) {
    return this.prisma.quizStructure.findMany({
      where: {
        id: { in: quizIds },
      },
      select: {
  ...
  async getContentCompletionStats(
    ...
    return {
      started: Number(result[0].started),
  ...
  async getUserEngagementMetrics(date: Date) {
    ...
    const returningUsers = await this.prisma.$queryRaw<Array<{ count: bigint }>>`
    ...
    const actionsPerUser = await this.prisma.$queryRaw<Array<{ avg_actions: number }>>`
