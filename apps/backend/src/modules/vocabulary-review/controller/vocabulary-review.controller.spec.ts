...
describe("VocabularyReviewController", () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: VocabularyReviewService, useValue: reviewServiceMock }],
    }).compile();

    controller = module.get<VocabularyReviewController>(VocabularyReviewController);
  });

  it("getReviewSession throws when childId missing", async () => {
    await expect(controller.getReviewSession({ user: {} } as any, undefined)).rejects.toThrow(
      BadRequestException,
    );
  });

  it("getReviewSession uses default limit 20", async () => {
    (reviewServiceMock.getReviewSession as any).mockResolvedValue({ items: [] });

    await controller.getReviewSession({ user: { childId: 3 } } as any, undefined);
  ...
  it("submitBulkReviews validates payload format", async () => {
    await expect(
      controller.submitBulkReviews({ user: { childId: 3 } } as any, { foo: [] } as any),
    ).rejects.toThrow(BadRequestException);
  });

  it("submitBulkReviews accepts array body", async () => {
    const reviews = [{ vocabularyId: 1, quality: 4 }];
    (reviewServiceMock.submitBulkReviews as any).mockResolvedValue([{ success: true }]);
    ...
    expect(reviewServiceMock.submitBulkReviews).toHaveBeenCalledWith(3, reviews);
