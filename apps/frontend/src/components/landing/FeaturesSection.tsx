        ...
        <section id="features" className="relative z-10 py-32 px-4 bg-background border-y border-border">
            <div className="max-w-7xl mx-auto">
                ...
                >
                    ...
                    <Heading level={2} className="font-heading font-black text-4xl sm:text-5xl text-heading mb-6">Trợ Thủ Đắc Lực Cho Bé</Heading>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 px-4 sm:px-0">
                    {/* Feature 1 */}
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} viewport={{ once: true }} className="h-full">
                        <SpotlightCard spotlightColor="rgba(16, 185, 129, 0.2)" className="h-full group p-8 sm:p-10 hover:border-success/50 transition-all duration-500 hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.2)] hover:-translate-y-2 flex flex-col">
                            ...
                            <Heading level={3} className="font-heading font-bold text-2xl text-heading mb-4 group-hover:text-success transition-colors">Vui Học Qua Trò Chơi</Heading>
                            <Body className="text-body text-lg leading-relaxed flex-grow">Ghi nhớ từ vựng siêu tốc thông qua hệ thống flashcard thông minh, game lật hình và câu đố tương tác vui nhộn.</Body>
                        </SpotlightCard>
                    </motion.div>

                    {/* Feature 2 */}
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} className="h-full">
                        <SpotlightCard spotlightColor="rgba(99, 102, 241, 0.2)" className="h-full group p-8 sm:p-10 hover:border-primary/50 transition-all duration-500 hover:shadow-[0_20px_40px_-15px_rgba(99,102,241,0.2)] hover:-translate-y-2 flex flex-col">
                            ...
                            <Heading level={3} className="font-heading font-bold text-2xl text-heading mb-4 group-hover:text-primary transition-colors">Công Nghệ AI Chấm Điểm</Heading>
                            <Body className="text-body text-lg leading-relaxed flex-grow">Không còn sợ nói sai! Trợ lý AI liên tục lắng nghe, phân tích và hướng dẫn phát âm chuẩn âm điệu người bản xứ.</Body>
                        </SpotlightCard>
                    </motion.div>

                    {/* Feature 3 */}
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} viewport={{ once: true }} className="h-full">
                        <SpotlightCard spotlightColor="rgba(244, 63, 94, 0.2)" className="h-full group p-8 sm:p-10 hover:border-accent/50 transition-all duration-500 hover:shadow-[0_20px_40px_-15px_rgba(244,63,94,0.2)] hover:-translate-y-2 flex flex-col">
                            ...
                            <Heading level={3} className="font-heading font-bold text-2xl text-heading mb-4 group-hover:text-accent transition-colors">Phần Thưởng Hấp Dẫn</Heading>
                            <Body className="text-body text-lg leading-relaxed flex-grow">Mỗi bài học hoàn thành là một rương báu mở ra! Sưu tầm sao vàng, mua đồ tân trang cho Avatar và đua Top ngay.</Body>
                        </SpotlightCard>
                    </motion.div>
                </div>
            </div>
