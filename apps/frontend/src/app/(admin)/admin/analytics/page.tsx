    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            ...
            try {
                ...
                const [dashboard, dauMetrics, sessionLength, contentPopularity] = await Promise.all([
                    getAdminDashboard(),
                ...
                const activeUserRate = dashboard.totalUsers > 0
                    ...
                    : 0;
                const activeChildRate = dashboard.totalChildren > 0
                    ? Math.round((dashboard.activeChildren / dashboard.totalChildren) * 100)
                    : 0;
                ...
                setData({
                    ...
                    funnel: [
                        ...
                        { label: 'Trẻ em hoạt động', value: activeChildRate, barColor: 'bg-success' },
                        { label: 'Tương tác', value: Math.round(dashboard.engagementRate), barColor: 'bg-warning' },
                    ],
                    gameModes: [],
                    kpis: [
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-card border-2 border-border rounded-2xl p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                            {data.gameModes.map((m, i) => (
                                <motion.div key={m.mode} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="bg-background border-2 border-border rounded-2xl p-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-card border-2 border-border rounded-2xl p-6">
                            ...
                            <div className="flex items-center gap-6 mt-4">
                                <DonutRing pct={Math.min(100, Math.max(0, Math.round(data.engagementRate)))} label="avg" />
                                <div className="space-y-3 flex-1">
                                    {[
                                        { label: 'Phiên học TB', value: `${data.averageSessionLength} phút` },
                                        { label: 'Tổng quiz', value: data.totalQuizzes.toLocaleString() },
                                        { label: 'Tương tác', value: `${data.engagementRate}%` },
                                    ].map((row) => (
                                        <div key={row.label}>
                        ...
                        {/* Gamification */}
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="bg-card border-2 border-border rounded-2xl p-6">
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-2">
                                {[
                                    ...
                                    <motion.div key={s.label} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="bg-background border-2 border-border rounded-xl p-3">
